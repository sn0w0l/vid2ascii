...
const uint64_t *tiles = char_list;
    ...
    for (size_t x = 0; x < w; x++) {
      ...
      for (size_t dy = 0; dy < 8; dy++) {
        for (size_t dx = 0; dx < 8; dx++) {
          ...
          const uint64_t huh = (tiles[intensity] & 1ULL << tile_pos);
    ...
    for (size_t x = 0; x < w; x++) {
      ...
      for (size_t dy = 0; dy < 8; dy++) {
        for (size_t dx = 0; dx < 8; dx++) {
          ...
          out_frame[dst + 0] = (tiles[r] & 1ULL << tile_pos) ? 255 : 0;
          out_frame[dst + 1] = (tiles[g] & 1ULL << tile_pos) ? 255 : 0;
          out_frame[dst + 2] = (tiles[b] & 1ULL << tile_pos) ? 255 : 0;
        }
      }
    }
  }
